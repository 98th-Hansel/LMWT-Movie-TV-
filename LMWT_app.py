...
def create_movie_tile(movie, click_handler, delete_handler=None):
    ...
    #tile.setCursor(QCursor(Qt.CursorShape.PointingHandCursor)) # Hand cursor on hover
...
def group_movies(movies):
    ...
    return grouped
#---------------------------------------------------------------------------------------------------
def create_movies_browse_page(movies_list, click_handler, title="Movies", switch_to_tv_callback=None, switch_to_home_callback=None):
    """
    ...
    right_side_layout.addWidget(settings_btn)
    ...
    scroll.setHorizontalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAsNeeded) # Change to ScrollBarAlwaysOff if i dont want horizontal scrolling
    ...
    grid_container.setMinimumWidth(COLUMNS * 215)  # 200px tile + 15px spacing per column
    ...
    return browse_page
#---------------------------------------------------------------------------------------------------
def create_tv_browse_page(tv_shows_list, click_handler, title="TV Shows", switch_to_movies_callback=None, switch_to_home_callback=None):
    """
...
def create_home_page(switch_to_movies_callback=None, switch_to_tv_callback=None, continue_watching_items=None, continue_click_handler=None, clear_all_callback=None, delete_item_callback=None):
    ...
    # Placeholder content
    placeholder_label = QLabel("Welcome to LMWT")
...
class VideoPlayer(QWidget):
    ...
    def create_overlay(self):
        ...
        # Play/Pause button
        self.play_pause_btn = QPushButton("⏸")  # Pause symbol
        ...
        if self.video_type == 'movie':
            self.episode_list_btn.setStyleSheet("""
                                                QPushButton {
                                                background-color: transparent;
                                                color: #555;
                                                border: none;
        ...
        if self.video_type == 'movie' or not self.next_episode_callback:
            self.next_episode_btn.setStyleSheet("""
                                                QPushButton {
                                                background-color: transparent;
                                                color: #555;
                                                border: none;
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Paths (temporary)
        movies_path = r"F:\Downloads\Movies"
        tv_path = r"F:\Downloads\TV Shows"

        self.movies = scan_movies(movies_path)
        self.tv_shows = scan_tv(tv_path)
    ...
    def on_continue_watching_click(self, item_list):
        ...
        if item['type'] == 'movie':
            # Play as movie
            self.player = VideoPlayer(item["path"], self.go_back, video_type='movie')
            ...
            self.player = VideoPlayer(
                ...
                self.go_back,
                video_type='episode',
                next_episode_callback=self.play_next_episode if hasattr(self, 'current_seasons_data') else None,
                episode_list_callback=self.get_episodes_data if hasattr(self, 'current_seasons_data') else None
            )
    ...
    def delete_progress_item(self, item):
        """Delete a single watch progress entry"""
